...
import { Plus, Search, Trash2, Mail, Briefcase, Shield, Edit2, Layers, Building2, ChevronDown, Check, X, Pause, Play } from 'lucide-react';
import { collection, onSnapshot, query, orderBy, deleteDoc, doc, where, updateDoc } from 'firebase/firestore';
import { db } from '../firebase';
import { toast } from 'sonner';
import { useAuth, isSuperUser, isSuperUser1, isSuperUser5, SUPER_ROLE_LABELS } from '../context/AuthContext';
...
const Users = () => {
    ...
    useEffect(() => {
        ...
        const unsubscribe = onSnapshot(q, (snapshot) => {
            ...
            if (activeEmpresa === 'Todas') {
                filtered = allUsers;
            } else if (activeEmpresa === 'ARTORIES - TUS HISTORIAS') {
                // Si estamos en la sede central (Artories), mostramos a los de Artories y a TODOS los Super Usuarios (para poder administrarlos)
                filtered = allUsers.filter(u => 
                    (Array.isArray(u.empresas) && u.empresas.includes(activeEmpresa)) || isSuperUser(u.role)
                );
            } else {
                filtered = allUsers.filter(u => 
    ...
    const filteredUsers = users.filter(u => {
        ...
        if (hasGlobalAccess) {
            // Usuarios con acceso global usan el filtro del select "empresaFilter"
            if (empresaFilter === "Todas") {
                matchesEmpresa = true;
            } else if (empresaFilter === "ARTORIES - TUS HISTORIAS") {
                // Si filtran por Artories, también les mostramos a los Super Usuarios
                matchesEmpresa = targetEmpresas.includes(empresaFilter) || isSuperUser(u.role);
            } else {
                matchesEmpresa = targetEmpresas.includes(empresaFilter);
            }
        } else {
            // El admin (y otros) solo ven los de su contexto (empresa activa) o de sus empresas asignadas
            if (activeEmpresa === "ARTORIES - TUS HISTORIAS") {
                matchesEmpresa = targetEmpresas.includes(activeEmpresa) || isSuperUser(u.role);
    ...
    const handleTogglePause = async (user) => {
        try {
            const newStatus = user.status === 'paused' ? 'active' : 'paused';
            await updateDoc(doc(db, "users", user.id), { status: newStatus });
            toast.success(`Usuario ${newStatus === 'paused' ? 'pausado' : 'activado'}`);
        } catch (error) {
    ...
    const handleDelete = async () => {
        ...
        try {
            ...
            // Si es Super Usuario Nivel 5 y yo soy Super Usuario Nivel 1
            if (isSuperUser5(targetUser.role) && isSuperUser1(currentUser?.role)) {
                if (targetUser.empresas && targetUser.empresas.length > 0) {
                    // Buscar todas las empresas asociadas a este Super Usuario 5
                    const { getDocs } = await import('firebase/firestore');
                    
                    // Firestore 'in' query supports up to 10 items. We assume targetUser.empresas <= 10.
                    // If more, we'd need to chunk, but we assume it's fine for now.
                    for (const empName of targetUser.empresas) {
                        const qEmp = query(collection(db, "empresas"), where("name", "==", empName));
                        const empSnap = await getDocs(qEmp);
                        
                        for (const empDoc of empSnap.docs) {
                            const empId = empDoc.id;
                            
                            // Solo borrar si es una empresa de un SuperUsuario 5
                            if (empDoc.data().isSuperUser5Company) {
                                // Borrar la empresa
                                await deleteDoc(doc(db, "empresas", empId));
                                
                                // Borrar áreas de esta empresa
                                const qAreas = query(collection(db, "areas"), where("empresaId", "==", empId));
                                const areasSnap = await getDocs(qAreas);
                                for (const aDoc of areasSnap.docs) {
                                    await deleteDoc(doc(db, "areas", aDoc.id));
                                }

                                // Borrar usuarios de esta empresa (excepto el propio SU5)
                                const qUsers = query(collection(db, "users"), where("empresas", "array-contains", empName));
                                const usersSnap = await getDocs(qUsers);
                                for (const uDoc of usersSnap.docs) {
                                    if (uDoc.id !== targetUser.id) {
                                        await deleteDoc(doc(db, "users", uDoc.id));
                                    }
                                }
                                
                                // Also delete collections where empresa == empName (simplified cleanup)
                                const collectionsToClean = [
                                    "campanas", "solicitudes_contenido", "brand_fonts", 
                                    "brand_survey_responses", "crm_deals", "clientes_frecuentes", 
                                    "ordenes_compra", "workflow_ai_chats", "workflow_ai_projects", 
                                    "cuadernos", "links", "cotizaciones", "finanzas_egresos"
                                ];
                                
                                for (const colName of collectionsToClean) {
                                    const qCol = query(collection(db, colName), where("empresa", "==", empName));
                                    const snapCol = await getDocs(qCol);
                                    for (const docCol of snapCol.docs) {
                                        await deleteDoc(doc(db, colName, docCol.id));
                                    }
                                }
                            }
                        }
    ...
    const showOnlineStatus = currentUser?.role === 'admin' || isSuperUser(currentUser?.role);
                            ...
                            <div key={user.id} className="bg-white p-6 rounded-xl shadow-sm border border-gray-100 hover:shadow-md transition-all relative group flex flex-col h-full min-h-[200px]">
                                ...
                                <div className="flex justify-between items-start mb-4 gap-2">
                                    <div className="relative">
                                        ...
                                        {showOnlineStatus && (
                                            <div 
                                                className={`absolute bottom-0 right-0 w-3.5 h-3.5 rounded-full border-2 border-white shadow-sm transition-colors ${user.isOnline ? 'bg-emerald-500' : 'bg-red-500'}`} 
                                                title={user.isOnline ? 'Usando la app' : 'Inactivo'}
                                            ></div>
                                        )}
                                    </div>
                                ...
                                <div className="absolute bottom-4 right-4 flex gap-2 z-10">
                                    ...
                                    */}
                                    {(() => {
                                        ...
                                        return (
                                            <>
                                                {isSuperUser5(user.role) && iAmSuper1 && (
                                                    <button onClick={() => handleTogglePause(user)} className={`p-2 bg-white ${user.status === 'paused' ? 'text-emerald-500 hover:text-emerald-600 hover:border-emerald-300' : 'text-amber-500 hover:text-amber-600 hover:border-amber-300'} border border-gray-200 rounded-full shadow-sm transition-all`} title={user.status === 'paused' ? 'Activar Usuario' : 'Pausar Usuario'}>
