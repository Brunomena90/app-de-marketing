...
const Login = () => {
    ...
    useEffect(() => {
        try {
            const saved = JSON.parse(localStorage.getItem('gci_saved_users') || '[]');
            setSavedUsers(saved);
        } catch (e) {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            const newSaved = [...new Set([email, ...savedUsers])].slice(0, 4);
            localStorage.setItem('gci_saved_users', JSON.stringify(newSaved));
                                ...
                                <div className="flex flex-wrap gap-2 mt-2">
                                    {savedUsers.map(u => (
                                        <button 
                                            ...
                                            type="button"
                                            onClick={() => setEmail(u)}
                                            className={`text-[10px] font-medium px-2.5 py-1 rounded-full transition-colors border ${email === u ? 'bg-blue-50 border-blue-200 text-blue-600' : 'bg-gray-50 border-gray-200 text-gray-500 hover:bg-gray-100'}`}
                                        >
                                            {u}
                                        </button>
                                    ))}
