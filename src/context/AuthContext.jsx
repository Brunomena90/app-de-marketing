...
export const isSuperUser5 = (role) => {
    const r = (role || '').toLowerCase();
    return r === 'superusuario_5';
};
...
export const AuthProvider = ({ children }) => {
    ...
    useEffect(() => {
        try {
            ...
            if (storedUser && storedUser !== "undefined" && storedUser !== "[object Object]") {
                ...
                // Lógica de empresa activa inicial
    ...
    // Escuchar cambios del usuario en tiempo real independientemente de si vino de localstorage o de un login reciente
    useEffect(() => {
        let unsubUser = null;
        
        if (user && (user.uid || user.id)) {
            unsubUser = onSnapshot(doc(db, 'users', user.uid || user.id), (docSnap) => {
                if (docSnap.exists()) {
                    ...
                    const updatedUser = {
                        ...
                        empresas: updatedData.empresas || [],
                        accessibleApps: updatedData.accessibleApps || [],
                    ...
                    // Solo actualizamos si hubo un cambio real para evitar renders infinitos
                    setUser(prev => {
                        if (JSON.stringify(prev) !== JSON.stringify(updatedUser)) {
                            localStorage.setItem('gci_user', JSON.stringify(updatedUser));
                            return updatedUser;
                        }
                        return prev;
                    });
    ...
    useEffect(() => {
        ...
        const updatePresence = async (status) => {
            try {
                ...
                // Ignore silent errors for presence
            }
        };
    ...
    const globalAccess = (isSuperUser(user?.role) && !isSuperUser5(user?.role)) || isArtoriesWorker;
