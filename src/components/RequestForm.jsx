...
const RequestForm = ({ onSuccess, initialDate, isOpen = true }) => {
    ...
    const handleRemoveBriefingItem = (indexToRemove) => {
        setBriefingItems(briefingItems.filter((_, idx) => idx !== indexToRemove));
    };

    const handleManualSubmit = async () => {
        const d = getValues();
        
        if (!d.title) return toast.error("El Título es requerido");
        if (!d.area) return toast.error("El Área es requerida");
        ...
        try {
            const base = {
                ...
                status: 'solicitado',
                applicantName: user?.name || 'Desconocido',
                applicantId: user?.uid || null,
                createdAt: new Date().toISOString(),
                requestDate: d.requestDate || new Date().toISOString().split('T')[0],
                deliveryDate: d.deadline || null,
            };

            if (activeTab === 'video') {
                base.objetivo = d.objective || '';
                base.publico = d.targetAudience || '';
                base.mensaje = d.keyMessage || '';
                base.briefing = briefingItems.join('\n');
                
                // Construct recordings array if dates are provided
                const recs = [];
                if (d.recordingStartDate) {
                    recs.push({
                        date: d.recordingStartDate,
                        startTime: d.recordingStartTime || '',
                        endTime: (d.recordingEndDate === d.recordingStartDate) ? (d.recordingEndTime || '') : ''
                    });
                }
                if (d.recordingEndDate && d.recordingEndDate !== d.recordingStartDate) {
                    recs.push({
                        date: d.recordingEndDate,
                        startTime: '',
                        endTime: d.recordingEndTime || ''
                    });
                }
                base.recordings = recs;
                base.format = d.format || "Horizontal (16:9)";
            } else {
                base.copy = d.additionalDetails || '';
            }
