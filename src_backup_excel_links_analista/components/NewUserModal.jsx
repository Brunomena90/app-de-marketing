...
import { ARTORIES_EMPRESA, SUPER_ROLES, SUPER_ROLE_LABELS, useAuth, isSuperUser1, isSuperUser } from '../context/AuthContext';
...
const NewUserModal = ({ isOpen, onClose, userToEdit = null, defaultEmpresa = null }) => {
    ...
    // Detectar si Artories está entre las empresas seleccionadas (habilita super roles preliminarmente)
    const artoriesSelected = selectedEmpresas.includes(ARTORIES_EMPRESA);
    // Solo un Super Usuario Nivel 1 puede asignar roles de Super Usuario
    const canAssignSuperRole = artoriesSelected && isSuperUser1(currentUser?.role);
    ...
    const onSubmit = async (data) => {
        ...
        try {
            const userData = {
                ...
                cargo: data.cargo,
                areas: data.areas || [],
                area: (data.areas && data.areas.length > 0) ? data.areas[0] : null,
                empresas: data.empresas || [],
                accessibleApps: isSuperUser(data.role) ? APP_MODULES.map(m => m.id) : (data.accessibleApps || []),
                    ...
                    <form id="userForm" onSubmit={handleSubmit(onSubmit)} className="space-y-4">
                        ...
                        <div className="pt-2 border-t border-gray-100">
                            ...
                            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                                {APP_MODULES.map(app => {
                                    const isSuper = isSuperUser(selectedRole);
                                    const isChecked = isSuper || selectedApps.includes(app.id);
                                    
                                    return (
                                        <div 
                                            key={app.id} 
                                            onClick={() => !isSuper && toggleApp(app.id)}
                                            className={`
                                                flex items-center gap-3 p-3 rounded-xl border transition-all cursor-pointer
                                                ${isChecked 
                                                    ? 'bg-blue-50 border-blue-200 text-blue-700 shadow-sm' 
                                                    : 'bg-gray-50 border-gray-100 text-gray-500 opacity-60 hover:opacity-100 hover:bg-white'}
                                                ${isSuper ? 'cursor-default ring-1 ring-blue-100' : 'active:scale-95'}
                                            `}
                                        >
                                            <div className={`
                                            ...
                                            {isSuper && <Lock size={12} className="ml-auto text-blue-400 opacity-50" />}
                                        </div>
                                    );
                                })}
                            </div>
                            {isSuperUser(selectedRole) && (
                                <p className="text-[10px] text-blue-500 mt-2 italic flex items-center gap-1">
                                    <Shield size={10} /> Los Super Usuarios tienen acceso total a todas las aplicaciones.
                                </p>
                            )}
                        </div>
                        ...
                        <div>
                            ...
                            {artoriesSelected && !isSuperUser1(currentUser?.role) && (
