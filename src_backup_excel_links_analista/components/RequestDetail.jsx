...
const RequestDetail = ({ request: initialRequest, isOpen, onClose }) => {
    ...
    const updateRecordingsDB = async (currentRecs) => {
        const filtered = currentRecs.filter(r => r.date);
        try {
                                ...
                                <Accordion title="Detalles" icon={FileText} defaultOpen={true}>
                                    <div className="space-y-4">
                                        <div>
                                            ...
                                            {canEdit ? (
                                                <textarea defaultValue={request.objetivo || ""} onBlur={e => updateField('objetivo', e.target.value)} onInput={autoResize} className="w-full text-sm bg-gray-50 focus:bg-white p-3 border border-transparent focus:border-blue-300 rounded-lg cursor-text outline-none resize-none overflow-hidden transition-colors" rows={1} placeholder="Añadir objetivo..." />
                                            ) : (
                                        ...
                                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                            <div>
                                                ...
                                                {canEdit ? (
                                                    <input defaultValue={request.publico || ""} onBlur={e => updateField('publico', e.target.value)} className="w-full text-sm bg-gray-50 focus:bg-white p-2.5 border border-transparent focus:border-blue-300 rounded-lg outline-none transition-colors" placeholder="Añadir público..." />
                                                ) : (
                                            ...
                                            <div>
                                                ...
                                                {canEdit ? (
                                                    <select value={request.format || "-"} onChange={e => updateField('format', e.target.value)} className="w-full text-sm bg-gray-50 focus:bg-white p-2.5 border border-transparent focus:border-blue-300 rounded-lg outline-none cursor-pointer transition-colors">
                                                        <option value="-">-</option>
                                        ...
                                        <div>
                                            <span className="block text-[10px] font-bold text-gray-400 uppercase mb-1 flex justify-between">
                                                Briefing
                                                {canEdit && <span className="text-[9px] text-gray-400 lowercase font-normal italic">Clic p/editar</span>}
                                            </span>
                                            {canEdit ? (
                                                <textarea defaultValue={request.briefing || ""} onBlur={e => updateField('briefing', e.target.value)} onInput={autoResize} className="w-full text-sm bg-blue-50/50 text-blue-900 focus:bg-white p-3.5 border border-transparent focus:border-blue-300 rounded-lg outline-none resize-none transition-colors leading-relaxed" rows={4} placeholder="Escribe el briefing..." />
                                            ) : (
                                        ...
                                        {(request.copy || canEdit) && (
                                            <div>
                                                ...
                                                {canEdit ? (
                                                    <textarea defaultValue={request.copy || ""} onBlur={e => updateField('copy', e.target.value)} onInput={autoResize} className="w-full text-sm bg-gray-50 focus:bg-white p-3.5 border border-transparent focus:border-blue-300 rounded-lg outline-none italic resize-none transition-colors" rows={3} placeholder="Añadir copy..." />
                                                ) : (
                                            ...
                                            <div key={i} className="flex flex-col sm:flex-row sm:items-start gap-3 p-3 border rounded-lg hover:border-blue-300 transition-colors group bg-white">
                                                ...
                                                <div className="flex items-center justify-end gap-2 w-full sm:w-auto pl-8 sm:pl-0">
                                                    <select disabled={!canEdit} value={t.type} onChange={(e) => changeTaskType(i, e.target.value)} className="text-xs border rounded-md px-2 py-1 bg-gray-50 focus:bg-white outline-none w-full sm:w-28 disabled:opacity-50 h-8 cursor-pointer hover:border-gray-300 transition-colors">
                                                        <option>Solo Video</option><option>Voz en Off</option><option>Escena Compuesta</option><option>Imagen</option><option>Diseño</option><option>Foto</option>
                                                    </select>
