...
const RequestDetailPage = () => {
    ...
    const handlePostProductionToggle = async (checked) => {
        try {
            ...
            if (checked && !request.postProductionStartDate) {
                dataToUpdate.postProductionStartDate = new Date().toISOString().split('T')[0];
            } else if (!checked) {
                dataToUpdate.postProductionStartDate = null;
            }
    ...
    const handlePostProductionFinish = async (checked) => {
        try {
            ...
            if (checked && !request.postProductionEndDate) {
                dataToUpdate.postProductionEndDate = new Date().toISOString().split('T')[0];
            } else if (!checked) {
                dataToUpdate.postProductionEndDate = null;
            }
            await updateDoc(doc(db, "solicitudes_contenido", request.id), dataToUpdate);
                                    ...
                                    {isProductionMode && (
                                        <div className="mt-6 pt-6 border-t border-slate-100 animate-in slide-in-from-bottom-4 duration-500 space-y-6">
                                            ...
                                            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                                                <label className={`flex items-start gap-3 p-4 rounded-2xl border cursor-pointer transition-all ${request.inPostProduction ? 'bg-indigo-50 border-indigo-200 shadow-sm' : 'bg-white border-slate-200 hover:border-slate-300'}`}>
                                                    <input 
                                                        type="checkbox" 
                                                        checked={!!request.inPostProduction} 
                                                        disabled={!canEdit}
                                                    ...
                                                    <div className="flex flex-col">
                                                        ...
                                                        {request.postProductionStartDate && <span className="text-[10px] font-bold text-indigo-600 mt-1 uppercase tracking-wider">Inicio: {request.postProductionStartDate}</span>}
                                                    </div>
                                                </label>
                                                ...
                                                <label className={`flex items-start gap-3 p-4 rounded-2xl border cursor-pointer transition-all ${request.postProductionFinished ? 'bg-emerald-50 border-emerald-200 shadow-sm' : 'bg-white border-slate-200 hover:border-slate-300'}`}>
                                                    ...
                                                    <div className="flex flex-col">
                                                        ...
                                                        {request.postProductionEndDate && <span className="text-[10px] font-bold text-emerald-600 mt-1 uppercase tracking-wider">Fin: {request.postProductionEndDate}</span>}
                                                    </div>
                                                </label>
                                                
                                                <label className={`flex items-start gap-3 p-4 rounded-2xl border cursor-pointer transition-all ${request.hasChanges ? 'bg-amber-50 border-amber-200 shadow-sm' : 'bg-white border-slate-200 hover:border-slate-300'}`}>
                                                    <input 
                                                        type="checkbox" 
                                                        checked={!!request.hasChanges} 
                                                        disabled={!canEdit}
                                                        onChange={(e) => updateField('hasChanges', e.target.checked)}
                                                        className="mt-0.5 w-4 h-4 text-amber-500 rounded border-amber-300 text-amber-600 focus:ring-amber-500" 
                                                    />
                                                    <div className="flex flex-col">
                                                        <span className="text-sm font-bold text-amber-900 leading-tight">Con Cambios</span>
                                                        <span className="text-[10px] text-amber-700/60 font-medium">Requiere ajustes en edición</span>
                                                    </div>
                                                </label>
                                            </div>
                                        </div>
